...
export class UIManager {
  ...
  // ... (Reszta pliku injectStyles, checkAdminPermissions itp. bez zmian - kopiuj z poprzedniej wersji)
  // Ze względu na limit znaków, wklejam tu tylko kluczowe zmiany. 
  // Upewnij się, że masz resztę metod z poprzedniej wersji (injectStyles, setupButtonHandlers itd.)
  // Poniżej wklejam tylko te metody, które są niezbędne dla kontekstu, resztę zostaw jak była.

  injectStyles() {
      const style = document.createElement('style');
      style.textContent = `
          /* MAIL STYLES Z POPRZEDNIEJ ODPOWIEDZI... */
          #mail-panel .panel-content { display: flex; flex-direction: row; width: 80vw; max-width: 800px; height: 70vh; padding: 0; overflow: hidden; background-color: rgba(0, 0, 0, 0.9); }
          .mail-sidebar { width: 35%; border-right: 1px solid #555; display: flex; flex-direction: column; background: rgba(255,255,255,0.05); }
          .mail-sidebar-header { padding: 15px; background: #2980b9; display: flex; justify-content: space-between; align-items: center; }
          #new-mail-btn { width: 30px; height: 30px; background: #27ae60; border-radius: 50%; display: flex; justify-content: center; align-items: center; cursor: pointer; font-size: 20px; }
          .mail-conversations { flex: 1; overflow-y: auto; }
          .mail-item { padding: 15px; border-bottom: 1px solid #444; cursor: pointer; transition: background 0.2s; }
          .mail-item:hover { background: rgba(255,255,255,0.1); }
          .mail-chat-view { flex: 1; display: flex; flex-direction: column; background: #222; }
          #mail-chat-header { padding: 10px; background: #2c3e50; border-bottom: 1px solid #555; text-align: center; }
          .mail-chat-messages { flex: 1; padding: 15px; overflow-y: auto; display: flex; flex-direction: column; gap: 10px; }
          .mail-message { padding: 8px 12px; border-radius: 15px; max-width: 70%; font-size: 14px; word-wrap: break-word; }
          .mail-message.sent { align-self: flex-end; background: #2980b9; color: white; }
          .mail-message.received { align-self: flex-start; background: #7f8c8d; color: white; }
          #mail-reply-form { display: flex; padding: 10px; background: #333; }
          #mail-reply-input { flex: 1; padding: 10px; border-radius: 20px; border: none; margin-right: 10px; }
          #mail-reply-btn { padding: 5px 15px; background: #27ae60; color: white; border: none; border-radius: 15px; cursor: pointer; }
          #new-mail-composer { position: absolute; top: 50%; left: 50%; transform: translate(-50%, -50%); background: #333; padding: 20px; border: 2px solid white; border-radius: 10px; width: 300px; display: none; flex-direction: column; gap: 10px; z-index: 100; }
  ...
  // Tutaj wklej resztę metod z poprzedniego ui.js (checkAdminPermissions, updatePlayerAvatar, setupButtonHandlers, friends, mail, discover...)
  // Aby kod działał, musisz mieć tu PEŁNĄ zawartość klasy UIManager z poprzedniej odpowiedzi + te 4 nowe metody do parkoura na górze.
  
  // (Dla pewności daję kluczowe metody, które muszą być):
  ...
  setupButtonHandlers() { document.querySelectorAll('.panel-close-button').forEach(btn => { btn.onclick = () => { const p = btn.closest('.panel-modal'); if(p) p.style.display = 'none'; }; }); document.querySelectorAll('.panel-content').forEach(c => c.addEventListener('click', e => e.stopPropagation())); document.querySelectorAll('.game-btn').forEach(button => { const type = this.getButtonType(button); button.addEventListener('click', () => this.handleButtonClick(type, button)); }); const pBtn = document.getElementById('player-avatar-button'); if (pBtn) pBtn.onclick = () => { this.openPanel('player-preview-panel'); if (this.onPlayerAvatarClick) this.onPlayerAvatarClick(); }; const friendsBtn = document.getElementById('btn-friends-open'); if (friendsBtn) { friendsBtn.onclick = () => { this.openPanel('friends-panel'); this.loadFriendsData(); }; } const topBarItems = document.querySelectorAll('.top-bar-item'); topBarItems.forEach(item => { if (item.textContent.includes('Poczta')) { item.onclick = () => { this.openPanel('mail-panel'); this.loadMailData(); }; } }); const chatToggle = document.getElementById('chat-toggle-button'); if (chatToggle) chatToggle.addEventListener('click', () => this.handleChatClick()); const setClick = (id, fn) => { const el = document.getElementById(id); if(el) el.onclick = fn; }; setClick('build-choice-new-world', () => { this.closePanel('build-choice-panel'); this.openPanel('world-size-panel'); }); setClick('build-choice-new-skin', () => { this.closePanel('build-choice-panel'); if(this.onSkinBuilderClick) this.onSkinBuilderClick(); }); setClick('build-choice-new-prefab', () => { this.closePanel('build-choice-panel'); if(this.onPrefabBuilderClick) this.onPrefabBuilderClick(); }); setClick('build-choice-new-part', () => { this.closePanel('build-choice-panel'); if(this.onPartBuilderClick) this.onPartBuilderClick(); }); setClick('size-choice-new-small', () => { this.closePanel('world-size-panel'); if(this.onWorldSizeSelected) this.onWorldSizeSelected(64); }); setClick('size-choice-new-medium', () => { this.closePanel('world-size-panel'); if(this.onWorldSizeSelected) this.onWorldSizeSelected(128); }); setClick('size-choice-new-large', () => { this.closePanel('world-size-panel'); if(this.onWorldSizeSelected) this.onWorldSizeSelected(256); }); setClick('toggle-fps-btn', () => { if(this.onToggleFPS) this.onToggleFPS(); }); const tabBlocks = document.getElementById('shop-tab-blocks'); const tabAddons = document.getElementById('shop-tab-addons'); if (tabBlocks && tabAddons) { tabBlocks.onclick = () => { tabBlocks.classList.add('active'); tabAddons.classList.remove('active'); this.shopCurrentCategory = 'block'; this.refreshShopList(); }; tabAddons.onclick = () => { tabAddons.classList.add('active'); tabBlocks.classList.remove('active'); this.shopCurrentCategory = 'addon'; this.refreshShopList(); }; } const nameSubmitBtn = document.getElementById('name-submit-btn'); if (nameSubmitBtn) { nameSubmitBtn.onclick = () => { const i = document.getElementById('name-input-field'); const v = i.value.trim(); if(v && this.onNameSubmit) { this.onNameSubmit(v); document.getElementById('name-input-panel').style.display = 'none'; } else alert('Nazwa nie może być pusta!'); }; } }
  ...
  handleButtonClick(buttonType, buttonElement) { buttonElement.style.transform = 'translateY(-1px) scale(0.95)'; setTimeout(() => { buttonElement.style.transform = ''; }, 150); if (buttonType === 'zagraj') { this.openPanel('discover-panel'); if (this.onPlayClick) this.onPlayClick(); return; } if (buttonType === 'buduj') { this.openPanel('build-choice-panel'); return; } if (buttonType === 'odkryj') { this.openPanel('discover-panel'); if (this.onDiscoverClick) this.onDiscoverClick(); return; } if (buttonType === 'wiecej') { this.openPanel('more-options-panel'); return; } if (buttonType === 'kup') { this.openPanel('shop-panel'); if (this.onShopOpen) this.onShopOpen(); return; } }
