...
export class UIManager {
  ...
  // --- PROFIL INNEGO GRACZA (NAPRAWIONE) ---
  async openOtherPlayerProfile(username) {
      ...
      try {
          // UŻYWAMY NOWEGO ENDPOINTU DO POBRANIA LEVELU I DATY
  ...
  // --- BUTTON HANDLERS (POPRAWIONE) ---
  setupButtonHandlers() {
    // 1. ZAMYKANIE PANELI (CLOSE X)
    ...
    // 2. KLIKNIĘCIE W TŁO (BACKGROUND CLICK)
    const moreOptions = document.getElementById('more-options-panel'); 
    if (moreOptions) { moreOptions.addEventListener('click', (e) => { if (e.target.id === 'more-options-panel') e.target.style.display = 'none'; }); }
    
    const profilePanel = document.getElementById('player-profile-panel'); 
    if (profilePanel) { profilePanel.addEventListener('click', (e) => { if (e.target.id === 'player-profile-panel') { profilePanel.style.display = 'none'; this.disposeCurrentPreview(); } }); }
    
    // NOWE: KLIKNIĘCIE W TŁO PROFILU INNEGO GRACZA
    ...
    topBarItems.forEach(item => { 
        if (item.textContent.includes('Poczta')) { item.onclick = () => { this.mailManager.open(); }; } 
    });
