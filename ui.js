...
export class UIManager {
  ...
  renderUI() {
      ...
      // Łączymy standardowe modale z nowym oknem skina
  ...
  // --- SZCZEGÓŁY SKINA (ZAMIAST SZYBKIEGO UBIERANIA) ---
  async showSkinDetails(item) {
      // item: {id, name, thumbnail, owner_id, creator, creatorLevel...}
      const modal = document.getElementById('skin-details-modal');
      if (!modal) {
          console.error("Brak HTML okna skina! Sprawdź UITemplates.js");
          return;
      }
      ...
      if(creatorLevel) creatorLevel.textContent = "1"; // Placeholder, dopóki API nie zwróci levela twórcy
      if(timeInfo) timeInfo.textContent = "niedawno"; // Placeholder daty

      // 2. Uruchom podgląd 3D
      ...
      if (btnLike) btnLike.onclick = () => { 
          alert("Funkcja lajkowania wkrótce!"); 
      };
  }
  ...
  async initSkinPreview3D(skinId) {
      ...
      this.skinPreviewCamera.position.set(0, 2, 9); // Odsunięta kamera
      this.skinPreviewCamera.lookAt(0, 1, 0);
      ...
      this.skinPreviewCharacter.position.y = -1.5; // Pozycja startowa
      ...
      if (skinData) {
          ...
          skinData.forEach(b => {
              ...
              // Pozycjonowanie klocków względem nóg
              mesh.position.set(b.x, b.y + 4.0, b.z); 
              this.skinPreviewCharacter.add(mesh);
          });
          // Skalowanie całości żeby się mieściło
          this.skinPreviewCharacter.scale.setScalar(0.6);
      }
  ...
  closeAllPanels() {
      // Zatrzymaj animację 3D skina, żeby nie zużywać baterii w tle
  ...
  setupButtonHandlers() {
    document.querySelectorAll('.panel-close-button').forEach(btn => {
        btn.onclick = () => { 
            ...
            // Specjalne czyszczenie dla okna skina
  ...
  // --- HELPERS (Kopiuj standardowe metody) ---
