...
export class UIManager {
  ...
  setupButtonHandlers() {
    ...
    // Zapobieganie zamykaniu modala przy kliknięciu wewnątrz
  ...
  populateShop(allBlocks, isOwnedCallback) {
    ...
    l.querySelectorAll('.buy-btn').forEach(btn => { 
        btn.onclick = () => { 
            const b = allBlocks.find(x => x.name === btn.dataset.blockName); 
  ...
  // FUNKCJA DO CZYSZCZENIA CZATU (Używana przez MultiplayerManager)
  clearChat() {
      const c = document.querySelector('.chat-area');
      if(c) c.innerHTML = '';
  }
  ...
  showMessage(text,type='info'){ 
      ...
      m.style.cssText=`position:fixed;top:50%;left:50%;transform:translate(-50%,-50%);background:${type==='success'?'#27ae60':(type==='error'?'#e74c3c':'#3498db')};color:white;padding:15px 25px;border-radius:10px;font-weight:bold;z-index:10000;box-shadow:0 6px 12px rgba(0,0,0,0.4);opacity:0;transition:all 0.3s ease;`; 
  ...
  async refreshSkinList(mode) { 
      ...
      this.populateDiscoverPanel('skins', skins, (skinId, skinName, thumbnail, ownerId)=>{ if(this.onSkinSelect) this.onSkinSelect(skinId, skinName, thumbnail, ownerId); }); 
  }
  ...
  populateDiscoverPanel(type, items, onSelect) { 
      ...
      items.forEach(item=>{ 
          ...
          if(type==='worlds'){ 
              if(typeof item==='object'){ label=item.name; if(item.creator) label+=` (od ${item.creator})`; thumbSrc=item.thumbnail; } else { label=item; } 
          } else { 
              label=item.name; if(item.creator) label+=` (od ${item.creator})`; thumbSrc=item.thumbnail; skinId=item.id; ownerId=item.owner_id; 
          } 
          ...
          div.onclick=()=>{ this.closeAllPanels(); if(type==='worlds') onSelect(item); else onSelect(skinId, item.name, item.thumbnail, ownerId); }; 
  ...
  async loadFriendsData() { 
      ...
      if(l) l.innerHTML='<p class="text-outline" style="text-align:center;margin-top:20px;">Odświeżanie...</p>'; 
      try{ 
          const r=await fetch(`${API_BASE_URL}/api/friends`,{headers:{'Authorization':`Bearer ${t}`}}); 
          if(r.ok){ 
              const d=await r.json(); 
              this.friendsList=d.friends; 
              this.renderFriendsList(d.friends); 
              this.renderRequestsList(d.requests); 
              this.updateTopBarFriends(d.friends); 
          } else if(l) l.innerHTML='<p class="text-outline" style="text-align:center;color:#e74c3c;">Błąd serwera.</p>'; 
      } catch(e){ if(l) l.innerHTML='<p class="text-outline" style="text-align:center;color:#e74c3c;">Błąd sieci.</p>'; } 
  }
  ...
  // Poprawiona struktura setupMailSystem
